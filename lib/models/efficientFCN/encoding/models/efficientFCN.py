...
class MHGDTwinLiteNet2ScaledTriLiteNet(nn.Module):
    def __init__(self, model_cfg):
        super(MHGDTwinLiteNet2ScaledTriLiteNet, self).__init__()

        # Ανάκτηση παραμέτρων από το model_cfg
        scale = model_cfg.get("scale", 1.0)
        num_center = model_cfg.get("num_center", 64)
        norm_layer = model_cfg.get("norm_layer", nn.BatchNorm2d)
        self.up_kwargs = model_cfg.get("up_kwargs", None)
        ...
        self.num_center = int(num_center * scale)
        self.hgdmodule0 = MultiHGDModuleTwinLiteNetv2Scaled(
    ...
    def forward(self, *inputs):
        feat_res8, feat_res16, feat_res32 = inputs[0][0]
        feat_32 = self.conv50(feat_res32)
        feat_16 = self.conv40(feat_res16)
        feat_8 = self.conv30(feat_res8)
        outs0 = self.hgdmodule0(feat_32, feat_16, feat_8)
